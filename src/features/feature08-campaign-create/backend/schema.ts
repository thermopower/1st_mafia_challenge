...
export const CampaignCreateInputSchema = z.object({
  title: z.string().refine((title) => {
    const result = validateStringLength(title, '체험단명', 1, 100);
    return result.isValid;
  }, {
    message: '체험단명은 1-100자 사이로 입력해주세요.'
  }),
  ...
  location: z.string().refine((location) => {
    const result = validateStringLength(location, '장소', 1, 255);
    return result.isValid;
  }, {
    message: '장소는 1-255자 사이로 입력해주세요.'
  }),
  recruitmentCount: z.number().min(1, '모집 인원은 최소 1명 이상이어야 합니다.').max(1000, '모집 인원은 최대 1000명까지 가능합니다.'),
  startDate: z.string().refine((date) => {
    ...
    message: '시작일은 오늘 이후 날짜만 선택 가능합니다.'
  }),
