...
import { CampaignStatusUpdateSchema, InfluencerSelectionSchema, InfluencerRejectionSchema, campaignAdminErrorCodes, type CampaignAdminErrorCode } from './schema';
import { getCampaignApplicants, selectInfluencers, rejectInfluencers, updateCampaignStatus } from './service';
...
export const registerCampaignAdminRoutes = (app: Hono<AppEnv>) => {
  ...
  // 인플루언서 반려
  app.post('/campaigns/:id/rejection', async (c) => {
    const logger = getLogger(c);
    const supabase = getSupabase(c);
    const campaignId = c.req.param('id');
    ...
    try {
      ...
      const body = await c.req.json();
      const parsed = InfluencerRejectionSchema.safeParse(body);
      if (!parsed.success) {
        return respond(c, failure(400, campaignAdminErrorCodes.REJECTION_UPDATE_FAILED, '입력값이 올바르지 않습니다.', parsed.error.format()));
      }

      const result = await rejectInfluencers(supabase, user.id, campaignId, parsed.data.rejectedInfluencerIds);
      return respond(c, result);
    } catch (error) {
      logger.error('Unexpected error during rejection', error);
      return respond(c, failure(500, 'INTERNAL_SERVER_ERROR', '서버 내부 오류가 발생했습니다.'));
    }
  });
};
